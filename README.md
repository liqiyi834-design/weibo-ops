...
这是一个面向少数自有/朋友账号的内容运营工作台，用于辅助热点选题、背景整理、风格化草稿生成、人工审核和复盘。AI 负责提高效率，人负责判断、修改和发布。

## 文档入口

本仓库已纳入 HotComment-AI 项目方案，用于把现有微博运营资料升级为“热点输入 + 知识库检索 + 风格化锐评草稿生成 + 风险审查”的 AI 内容生产系统。

项目不做矩阵号、刷量、自动发博、自动评论、批量互动或绕过平台风控；所有输出默认进入候选池或草稿状态，由人工审核后再决定是否发布。

- `AGENTS.md`：当前 Agent 协作入口、硬规则、项目摘要和常用命令。
- `docs\README.md`：项目文档索引。
- `docs\agent_handbook\README.md`：当前交接分卷入口。
- `docs\HotComment-AI技术方案.md`：历史完整方案 / 同步整理版，保留产品和技术背景。
- `docs\legacy_ops\README.md`：早期人工运营资料归档说明。

如历史方案与当前实现冲突，以 `AGENTS.md` 和 `docs\agent_handbook\` 当前分卷为准。

早期人工运营资料已归档到 `docs\legacy_ops\`。其中 `04_人设与风格规则.md`、`06_草稿生成提示词.md`、`10_爆款博文写作公式.md`、`12_事实核查与风险分级.md` 等仍可作为 RAG、Prompt 和安全规则的知识来源。
...
1. 打开微博热搜，记录 15-30 个候选热点到 `docs\legacy_ops\01_热搜追踪模板.md`。
2. 从候选热点里挑 10-15 个，填入 `docs\legacy_ops\02_选题库.csv`。
3. 用 `docs\legacy_ops\03_每日十条草稿模板.md` 逐条写草稿。
4. 对照 `docs\legacy_ops\04_人设与风格规则.md` 做一轮润色。
5. 最后用 `docs\legacy_ops\05_发布复盘.md` 记录发布时间、互动数据和下次优化点。
...
- `docs\legacy_ops\08_高热博文公开样本研究.md`：公开高热博文写法拆解
- `docs\legacy_ops\09_评论区互动SOP.md`：发布后如何拉评论、接评论、控节奏
- `docs\legacy_ops\10_爆款博文写作公式.md`：社会、科技、娱乐、科普等模板公式
- `data\public_weibo_research_samples.csv`：公开样本来源与可复制点

## 运营前置条件

- `docs\legacy_ops\11_自媒体前置条件清单.md`：账号边界、发布节奏、复盘制度
- `docs\legacy_ops\12_事实核查与风险分级.md`：事实核查、来源等级、风险表达
- `docs\legacy_ops\13_账号运营周计划.md`：每日、每周、每月运营动作
- `docs\legacy_ops\14_发布排期.csv`：每天 10 条微博的排期表
- `docs\legacy_ops\15_素材来源与引用规范.md`：配图、截图、引用规范
- `docs\legacy_ops\16_增长指标看板.csv`：粉丝、阅读、互动数据记录
- `docs\legacy_ops\17_内容栏目设计.md`：固定栏目和标签池
- `docs\legacy_ops\18_账号视觉与主页装修.md`：昵称、简介、头像、置顶微博、背景图
- `docs\legacy_ops\19_每日执行清单.md`：每天从热搜到复盘的完整步骤
- `docs\legacy_ops\20_运营交接与权限清单.md`：我和你分别负责什么、需要什么权限
- `docs\legacy_ops\21_运营控制台.md`：每天运营的总控面板
- `docs\legacy_ops\22_Edge微博样本采集助手.md`：用已登录 Edge 安全导出可见微博样本
- `docs\legacy_ops\23_Telegram拉取说明.md`：只保留 Telegram 消息手动拉取能力
- `docs\legacy_ops\24_高互动正文分析标准.md`：围绕正文、日期、转评赞分析高互动样本
