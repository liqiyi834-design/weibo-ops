...
class ZhihuTopicFitService:
    def evaluate(self, topic: HotTopic | HotSearchItem, category: str, risk_level: str) -> ZhihuTopicFit:
        ...
        return ZhihuTopicFit(
            ...
            answer_angle=self._answer_angle(keyword, category, risk_level),
    ...
    def _answer_angle(self, keyword: str, category: str, risk_level: str) -> str:
        if risk_level == "high":
            return "从公开事实、信息边界和讨论风险切入，避免对个人或案件下定论。"
        if category == "brand_pr":
            return "从品牌沟通、用户预期和公关失误的机制角度展开。"
        if category == "social_issue":
            return "从规则设计、普通人体验和平台治理角度展开。"
        if category == "gender_issue":
            return "从权责分配、社会期待和讨论边界角度展开。"
        if category == "entertainment":
            return "从作品/节目机制、公众情绪和传播反差角度展开，避免粉圈攻击。"
